...
@push('js')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Event listener untuk tombol Submit dengan SweetAlert
            document.getElementById('submit-form').addEventListener('submit', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        this.submit(); // Jalankan submit jika dikonfirmasi
                    }
                });
            });
            });
    ...
    <div class="container-fluid">
        ...
        @foreach ($detailKriteria as $index => $detail)
            <div class="category-section">
                    ...
                    <form id="add-form-{{ $index }}" class="data-pendukung-form-add add-form" data-category="{{ $index }}">
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="data_pendukung[{{ $index }}][new][nama_data]"
                                    rows="2"></textarea>
                                <input type="hidden" name="data_pendukung[{{ $index }}][new][id_detail_kriteria]"
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control ckeditor"
                                    name="data_pendukung[{{ $index }}][new][deskripsi_data]"></textarea>
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label>URL Data Pendukung</label>
                                <textarea class="form-control" name="data_pendukung[{{ $index }}][new][hyperlink_data]"
                                    rows="2"></textarea>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="data-pendukung-cards">
                        @foreach ($detail->dataPendukung as $dataIndex => $dataPendukung)
                            <div class="data-pendukung-card">
                                ...
                                @if ($kriteria->status_selesai != 'Submitted')
                                    <button class="btn btn-warning edit-data-btn" data-category="{{ $index }}"
                                        data-index="{{ $dataIndex }}">Edit</button>
                                @endif
                            </div>

                            <!-- Form edit data pendukung (tersembunyi) -->
                            @if ($kriteria->status_selesai != 'Submitted')
                                <form id="edit-form-{{ $index }}-{{ $dataIndex }}"
                                    class="data-pendukung-form edit-form edit-form-{{ $index }}" data-category="{{ $index }}">
                                    <input type="hidden" name="data_pendukung[{{ $index }}][{{ $dataIndex }}][id_data_pendukung]"
                                        value="{{ $dataPendukung->id_data_pendukung }}">
                                    <div class="form-row">
                                        <div class="form-group">
                                            <label>Nama Data Pendukung</label>
                                            <textarea class="form-control" name="data_pendukung[{{ $index }}][{{ $dataIndex }}][nama_data]"
                                                rows="2">{{ $dataPendukung->nama_data }}</textarea>
                                            <input type="hidden" name="data_pendukung[{{ $index }}][{{ $dataIndex }}][id_detail_kriteria]"
                                                value="{{ $detail->id_detail_kriteria }}">
                                            <input type="hidden" name="data_pendukung[{{ $index }}][{{ $dataIndex }}][index]"
                                                value="{{ $dataIndex }}">
                                        </div>
                                        <div class="form-group">
                                            <label>Deskripsi Data Pendukung</label>
                                            <textarea class="form-control ckeditor"
                                                name="data_pendukung[{{ $index }}][{{ $dataIndex }}][deskripsi_data]">{{ $dataPendukung->deskripsi_data }}</textarea>
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control"
                                                name="data_pendukung[{{ $index }}][{{ $dataIndex }}][hyperlink_data]"
                                                rows="2">{{ $dataPendukung->hyperlink_data }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label>Upload Data</label>
                                                ...
                                                <div class="existing-images" style="margin-bottom: 10px;">
                                                    @foreach ($dataPendukung->gambar as $gambar)
                                                        <div style="display: inline-block; position: relative; margin-right: 10px;">
                                                            ...
                                                            <a href="#" class="remove-image" data-id="{{ $gambar->id_gambar }}"
                                                                style="position: absolute; top: 0; right: 0; color: red; font-weight: bold; text-decoration: none; background: #fff; border-radius: 50%; padding: 0 5px;">X</a>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            @endif
                ...
                @if ($kriteria->status_selesai != 'Submitted')
                    <button type="button" class="btn btn-warning add-data-btn" data-category="{{ $index }}">Tambah Data
                        Pendukung</button>
                @endif
            </div>
        @endforeach
            ...
            <div class="category-section">
                ...
                @foreach ($komentar as $komen)
                    <div class="komentar-item"
                        style="background: #fff; padding: 10px; border-radius: 5px; margin-bottom: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
        ...
        @if ($kriteria->status_selesai != 'Submitted')
            <div class="action-buttons">
                <form action="{{ route('kriteria.save', ['id' => $kriteria->id_kriteria]) }}" method="POST"
                    style="display: inline;">
                    @csrf
                    <input type="hidden" name="final_save" value="1">
                    <button type="submit" class="btn btn-primary">Save</button>
                </form>
                <form action="{{ route('kriteria.submit', ['id' => $kriteria->id_kriteria]) }}" method="POST" id="submit-form"
                    style="display: inline;">
                    @csrf
